...
namespace MAS
{
    public class GameScreen
    {
        ...
        public void SpawnPlatform(GameTime gameTime)
        {
            ...
            {
                this.platform = new Platform(new DrawablePhysicsObject(this.world, this.Content.Load<Texture2D>("Pictures/Platform"), new Vector2(400, 50), 1F));
        ...
        public void Update(GameTime gameTime)
        {
            ...
            foreach (Platform platform in this.list_Platform)
            {
                platform.Update(gameTime, this.hud.Score);
            }
            this.player.Update(gameTime, this.curKeyboardState, this.prevKeyboardState, list_Platform, this.hud.Score);
            this.HandlePlatform();

            //Console.WriteLine(this.f_platformTimeInterval);
            ...
            // Snelheid van platformen spawnen
            foreach (Platform platform in this.list_Platform)
            {
                if (platform.platformSpeed != this.f_prevSpeed && platform.platformSpeed != 40)
                    this.f_platformTimeInterval -= 0.05F;
                this.f_prevSpeed = platform.platformSpeed;
            }
